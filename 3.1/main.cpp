...
complex<double> * FFT( complex<double> * polynom, int degree )
{
	...
	complex<double> * pol1 = new complex<double>[degree / 2 + 1];
	complex<double> * pol2 = new complex<double>[degree / 2 + 1];
	...
	complex<double> * val1 = new complex<double>[degree / 2 + 1];
	complex<double> * val2 = new complex<double>[degree / 2 + 1];
...
{
	complex<double> * pol1 = new complex<double>[degree];
	complex<double> * pol2 = new complex<double>[degree];
	complex<double> * pol3 = new complex<double>[degree];

	for (int i = 0; i < degree; i++)
	{
		pol1[i] = complex<double>(str1[i], 0);
		pol2[i] = complex<double>(str2[i], 0);
	}
	
	complex<double> * val1 = new complex<double>[degree];
	complex<double> * val2 = new complex<double>[degree];
	complex<double> * val3 = new complex<double>[degree];

	val1 = FFT(pol1, degree);
	val2 = FFT(pol2, degree);

	for (int i = 0; i < degree; i++)
		val3[i] = val1[i] * val2[i];

	pol3 = IFFT(val3, degree);

	char * str3 = new char[degree];
	
	for (int i = 0; i < degree; i++)
		str3[i] = (char)real(pol3[i]);
